...
const mapTilesToggle = document.getElementById("maptiles-toggle") as
  | HTMLInputElement
  | null;
if (mapTilesToggle) {
...
const navmeshToggle = document.getElementById("navmesh-toggle") as
  | HTMLInputElement
  | null;
if (navmeshToggle) {
...
const regionToggle = document.getElementById("region-info-toggle") as
  | HTMLInputElement
  | null;
if (regionToggle) {
...
for (let i = 0; i <= 7; i++) {
  const toggle = document.getElementById(`toggle-conn-${i}`) as
    | HTMLInputElement
    | null;
  if (toggle) {
...
if (toggleAllBtn) {
  toggleAllBtn.addEventListener("click", () => {
    ...
    for (let i = 0; i <= 7; i++) {
      const cb = document.getElementById(`toggle-conn-${i}`) as
        | HTMLInputElement
        | null;
      if (cb) {
        checkboxes.push(cb);
...
const layerSelect = document.getElementById("layer-select") as
  | HTMLSelectElement
  | null;
if (layerSelect) {
...
const searchInput = document.getElementById("search-input") as
  | HTMLInputElement
  | null;
const searchResults = document.getElementById("search-results") as
  | HTMLDivElement
  | null;
  ...
  Object.entries(groups).forEach(([categoryName, items]) => {
    ...
    itemsToShow.forEach((item) => {
      ...
      itemDiv.addEventListener("click", () => {
        ...
        // Show popup
        const regionString = targetLayerKey === "world"
          ? `${region} (${region & 0xff},${region >> 8})`
          : `${region}`;
    ...
    if (!query && items.length > 100) {
      ...
      moreDiv.textContent = `... and ${
        items.length - 100
      } more (use search to filter)`;
...
async function updateCacheStatusIcon() {
  ...
  precacheIcon.textContent = allCached
    ? "check_box"
