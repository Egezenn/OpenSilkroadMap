    ...
    if (src === 'dummy') {
      ...
      getPMTilesForLayer(currentLayerKey).then((pmtilesInstance) => {
        pmtilesInstance.getZxy(z, x, y)
          ...
          .catch(() => {
            (tile.getImage() as HTMLImageElement).src = 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7';
          });
      });
    }
  },
...
export function updateCoordsVal() {
  ...
  if (lastCoordinate) {
    ...
    const regionString = currentLayerKey === 'world'
...
map.on('pointermove', (event) => {
  ...
  hoverTimeout = window.setTimeout(() => {
    ...
    const isClickable = hitFeature && (hitFeature.getGeometry()?.getType() === 'Point' || hitFeature.get('isConnection'));
